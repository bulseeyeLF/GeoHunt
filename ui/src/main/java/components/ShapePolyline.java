...
public class ShapePolyline extends Shapes {
    @Getter
    @Setter
    ...
    public ShapePolyline(double ...coordinates){
        super();
        initPolygon(coordinates);
    }

    public ShapePolyline(JSONObject jsonObject) {
        JSONArray jsonArray = jsonObject.getJSONArray("coordinates");
        this.coordinates = new ArrayList<>();
        for (int i = 0; i < jsonArray.length() ; i ++){
            if (shape == null){
                initPolygon(
                    jsonArray.getJSONObject(0).getDouble("x"),
                    jsonArray.getJSONObject(0).getDouble("y"));
            } else {
                this.addPoint(
